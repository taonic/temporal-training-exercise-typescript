...
Edit the files under `exercise5/src/`, then run the Worker and Starter as shown in the final step. Each step below tells you which file to edit, what code to add, and why.

## Step 1 — Give each Activity its own proxy with a summary

**File:** open `exercise5/src/workflow.ts`.
...
Open the Temporal Web UI at http://localhost:8233 and look at the Activity events — each now
...
Stuck? Compare with the completed code in the `solution5/` directory.
